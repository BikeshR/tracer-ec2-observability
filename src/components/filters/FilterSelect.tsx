...
const FilterSelect: React.FC<FilterSelectProps> = ({ className = "" }) => {
  ...
  return (
    <div className={`flex items-center space-x-3 ${className}`}>
          ...
          {filterState.filterSets.map((filterSet) => (
            <SelectItem key={filterSet.id} value={filterSet.id}>
              <div className="flex items-center space-x-2">
                ...
                {/* Show filter details */}
                {(filterSet.filters.teams.length > 0 || filterSet.filters.regions.length > 0) && (
                  <span className="text-xs text-muted-foreground ml-2">
                    ({filterSet.filters.teams.length > 0 && `${filterSet.filters.teams.length} teams`}
                    {filterSet.filters.teams.length > 0 && filterSet.filters.regions.length > 0 && ", "}
                    {filterSet.filters.regions.length > 0 && `${filterSet.filters.regions.length} regions`})
                  </span>
                )}
              </div>
            </SelectItem>
      ...
      {activeFilterSet && (activeFilterSet.filters.teams.length > 0 || activeFilterSet.filters.regions.length > 0) && (
